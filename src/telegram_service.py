...
def get_chat_id_by_city(city):
    ...
    if "кемерово" in city and kem_chat: return kem_chat
    elif "красноярск" in city and kras_chat: return kras_chat
    elif "шерегеш" in city and shere_chat: return shere_chat
    else: return None
...
def send_order(order):
    ...
    # 1. Читаем переменную главного чата
    ...
    if main_chat_id:
        ...
        if success_main:
            ...
            print("❌ Ошибка отправки в ГЛАВНЫЙ чат (Неверный ID или бот не добавлен в чат?)")
    else:
